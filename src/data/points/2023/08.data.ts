import { IPointEntities } from 'store/reducers';
import { BallType } from 'types/ball.types';
import { GameType } from 'types/game.types';
import { MethodType } from 'types/method.types';

export const pointsData2023_8: IPointEntities = {
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0779. Bruxish
  '53d7a928-974b-48e3-a51d-7d9c010fede7': {
    data: {
      id: '53d7a928-974b-48e3-a51d-7d9c010fede7',
      type: 'point',
      attributes: {
        ball: 'repeat' as BallType,
        catchDate: '2023-04-16',
        firstCatch: true,
        game: 'violet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'd06db98e-bac0-4676-bf64-0cb0101ef847',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  james (@seginus)
  //  0973. Flamigo
  'ddf959f7-9dda-41cd-a7f3-767c2a87f2a7': {
    data: {
      id: 'ddf959f7-9dda-41cd-a7f3-767c2a87f2a7',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'violet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'c159b074-9e9f-4e1b-889f-a64dd78598a6',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'c2443651-d521-4dcd-8539-a00b84a4efaf',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Robert Lewis (@A1966Mustang)
  //  0739. Crabrawler
  '0faf1c28-48d4-449d-9ad0-19b3b9b1adcb': {
    data: {
      id: '0faf1c28-48d4-449d-9ad0-19b3b9b1adcb',
      type: 'point',
      attributes: {
        ball: 'quick' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '3657e524-63a2-4eb6-b768-c20b104e41a1',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '97bcb939-d1b2-4541-a03f-ef2b6af51ecf',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  1005. Roaring Moon
  '14400b7c-3a95-4bb5-8071-0657eae98d17': {
    data: {
      id: '14400b7c-3a95-4bb5-8071-0657eae98d17',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'c291dd7a-074f-4a64-831d-e759beedb893',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Lauren S (@Lauren S)
  //  0928. Smoliv
  '2f810551-ab82-42ad-9230-36fc28ad1156': {
    data: {
      id: '2f810551-ab82-42ad-9230-36fc28ad1156',
      type: 'point',
      attributes: {
        ball: 'friend' as BallType,
        catchDate: '2023-04-17',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6a3c8584-d4a7-45b9-8cb5-43f37527f3ba',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '31b03b29-3173-4950-91e9-0e8aeb568952',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0734. Yungoos
  '547ec7ed-8925-4fed-90ba-d662fd95a63a': {
    data: {
      id: '547ec7ed-8925-4fed-90ba-d662fd95a63a',
      type: 'point',
      attributes: {
        ball: 'pokeball' as BallType,
        catchDate: '2023-04-18',
        firstCatch: false,
        game: 'pogo' as GameType,
        method: 'pogoRandom' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'd1185462-764d-4cd3-90cb-932c34cca0e8',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Kathryn (@SteeleTheWin)
  //  0960. Wiglett
  'd345c286-ef5b-41a4-b360-f281b02701aa': {
    data: {
      id: 'd345c286-ef5b-41a4-b360-f281b02701aa',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-17',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'ec689f25-fffe-4249-b89d-603d574bacee',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'b64a786c-16b2-49b4-a507-5debb67a8ae5',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0765. Oranguru
  '138bc42e-dde2-45d9-969d-c0bf1b2c8624': {
    data: {
      id: '138bc42e-dde2-45d9-969d-c0bf1b2c8624',
      type: 'point',
      attributes: {
        ball: 'heal' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '65cb7a97-ff74-4733-932b-a0e4ac7cdfdf',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0987. Flutter Mane
  '108943db-033f-41bf-95b1-dbba2d0b39fa': {
    data: {
      id: '108943db-033f-41bf-95b1-dbba2d0b39fa',
      type: 'point',
      attributes: {
        ball: 'dusk' as BallType,
        catchDate: '2023-04-17',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'a711e10f-5109-43b8-8991-b826dcf1ce58',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0984. Great Tusk
  '55dd54ba-1004-4190-aacd-846a5e0048a1': {
    data: {
      id: '55dd54ba-1004-4190-aacd-846a5e0048a1',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '1f773ab0-6d6f-4853-a4a4-e174c167286f',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0634. Zweilous
  '6ebb80ba-38c5-4b17-8c84-0639c806ae07': {
    data: {
      id: '6ebb80ba-38c5-4b17-8c84-0639c806ae07',
      type: 'point',
      attributes: {
        ball: 'dusk' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'f5374f2f-62e7-4881-9cbc-a12c18c37050',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0985. Scream Tail
  'ceb63057-a2c0-4110-b119-ada9c1a3123a': {
    data: {
      id: 'ceb63057-a2c0-4110-b119-ada9c1a3123a',
      type: 'point',
      attributes: {
        ball: 'heal' as BallType,
        catchDate: '2023-04-16',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '191d3a48-0697-4a8c-9f20-25d0de2e1e05',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0972. Houndstone
  'b8c90c0e-e99a-4ff2-84f2-31e1f03cf8c7': {
    data: {
      id: 'b8c90c0e-e99a-4ff2-84f2-31e1f03cf8c7',
      type: 'point',
      attributes: {
        ball: 'quick' as BallType,
        catchDate: '2023-04-17',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: null,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '88183b39-8f47-4cb5-aef0-a60aff9aad59',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0741. Oricorio
  '44e40d70-3948-4156-83b2-f30d4f91e76a': {
    data: {
      id: '44e40d70-3948-4156-83b2-f30d4f91e76a',
      type: 'point',
      attributes: {
        ball: 'heal' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'ded7e96b-b368-4269-aeb9-22934ffe30c9',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0935. Charcadet
  '24ec95ca-624f-4763-ab61-766bb9d869af': {
    data: {
      id: '24ec95ca-624f-4763-ab61-766bb9d869af',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'cf3736c5-0795-49e3-b1e0-bb464936465d',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0979. Annihilape
  '65fa25a3-41c1-48a4-9d01-299311aa501b': {
    data: {
      id: '65fa25a3-41c1-48a4-9d01-299311aa501b',
      type: 'point',
      attributes: {
        ball: 'dusk' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '8833b9a4-399e-4a43-a864-3e1b0393fd01',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Tone (@Tone)
  //  0992. Iron Hands
  'a003f658-bc8e-4074-ae02-0a2dd730ba65': {
    data: {
      id: 'a003f658-bc8e-4074-ae02-0a2dd730ba65',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '6b7a4cdb-fd7b-448c-9f03-2b49b4ab3b9d',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'b9e7c43e-2141-4669-8b3c-a1f367d94c91',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0989. Sandy Shocks
  '5bdcb1d9-7f11-4e90-8c29-ac40b5d2cb61': {
    data: {
      id: '5bdcb1d9-7f11-4e90-8c29-ac40b5d2cb61',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '97fcd9cb-845a-4c0a-9e42-192be7012d96',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Padukes (@Padukes)
  //  0988. Slither Wing
  '50b6ee9b-5e9e-435f-96e8-2ddb9bb33491': {
    data: {
      id: '50b6ee9b-5e9e-435f-96e8-2ddb9bb33491',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1ca0b66b-2791-41c2-bf6d-aeabae1cfdb0',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '96348370-cf6d-4674-8c33-d310f34a8708',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Devilish Inferno (@Alex)
  //  0942. Maschiff
  '0b0ef636-9311-4472-9b6b-fdc74800aec8': {
    data: {
      id: '0b0ef636-9311-4472-9b6b-fdc74800aec8',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-22',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '3be16e09-d7c4-4ea5-a5bc-8fb3366355b6',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '6bd0e7ce-85ac-459a-b37d-426a0ffe2dc4',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Brandon Hero of Ice (@Brandon Heroice18)
  //  0932. Nacli
  '45d1ac77-6d74-4dba-a39a-d717995b7491': {
    data: {
      id: '45d1ac77-6d74-4dba-a39a-d717995b7491',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-24',
        firstCatch: false,
        game: 'violet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '1e665730-88cd-4ca3-b03c-961c3a71e749',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '5a857c69-3a7f-425a-82eb-a0a9fc0187f3',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Lewis Dobie (@Lewthelegend)
  //  0998. Baxcalibur
  'd794ecab-db2d-46ce-81f9-3de455e1f8b8': {
    data: {
      id: 'd794ecab-db2d-46ce-81f9-3de455e1f8b8',
      type: 'point',
      attributes: {
        ball: 'dive' as BallType,
        catchDate: '2023-04-18',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '7d054896-a0ea-4368-bff1-856b6abf8419',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'f612cbe0-8b28-4f51-9822-7969c039038d',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Lewis Dobie (@Lewthelegend)
  //  0979. Annihilape
  'ff3c0f67-282f-480c-8178-e355af63a496': {
    data: {
      id: 'ff3c0f67-282f-480c-8178-e355af63a496',
      type: 'point',
      attributes: {
        ball: 'dive' as BallType,
        catchDate: '2023-04-17',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '7d054896-a0ea-4368-bff1-856b6abf8419',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '8833b9a4-399e-4a43-a864-3e1b0393fd01',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Stuart (@stuart)
  //  0991. Iron Bundle
  '400e5aca-99d8-439b-ad93-a9efd4032803': {
    data: {
      id: '400e5aca-99d8-439b-ad93-a9efd4032803',
      type: 'point',
      attributes: {
        ball: 'pokeball' as BallType,
        catchDate: '2023-04-29',
        firstCatch: false,
        game: 'violet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '80cfecae-ef2e-437b-bb94-f0309ee3b3d2',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'd067ea89-4cef-4a4e-9331-2d031120a807',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Devilish Inferno (@Alex)
  //  0990. Iron Treads
  '05871482-6ab1-4912-b5ce-b9c85e350369': {
    data: {
      id: '05871482-6ab1-4912-b5ce-b9c85e350369',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-24',
        firstCatch: false,
        game: 'violet' as GameType,
        method: '—' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: '3be16e09-d7c4-4ea5-a5bc-8fb3366355b6',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '47c4097f-f029-4b07-913e-d1eb482a4824',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Kathryn (@SteeleTheWin)
  //  0664. Scatterbug
  '39d28abd-ff26-44d3-a401-31d7ffdd3796': {
    data: {
      id: '39d28abd-ff26-44d3-a401-31d7ffdd3796',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-28',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'ec689f25-fffe-4249-b89d-603d574bacee',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'de189ff7-1bf7-45ae-a49d-92c28217d44e',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Kathryn (@SteeleTheWin)
  //  0996. Frigibax
  '5873686f-32f9-4163-97f9-353ca0bb2468': {
    data: {
      id: '5873686f-32f9-4163-97f9-353ca0bb2468',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-28',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'ec689f25-fffe-4249-b89d-603d574bacee',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '9f8c7a79-1fcc-4c5c-95b5-a6e49d170c5b',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Kathryn (@SteeleTheWin)
  //  0917. Tarountula
  'cbab4f27-621a-4ae6-8543-656e61eaa4c2': {
    data: {
      id: 'cbab4f27-621a-4ae6-8543-656e61eaa4c2',
      type: 'point',
      attributes: {
        ball: 'luxury' as BallType,
        catchDate: '2023-04-28',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'ec689f25-fffe-4249-b89d-603d574bacee',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: 'ff315a13-b4dc-4601-aca3-13a16fda7a22',
            type: 'pokemon',
          },
        },
      },
    },
  },
  //  Pokémon on Switch for the First Time 16 Apr 2023  to 29 Apr 2023
  //  Al McKinlay (@Al)
  //  0915. Lechonk
  'b68ed500-f80f-4382-a026-20707916f741': {
    data: {
      id: 'b68ed500-f80f-4382-a026-20707916f741',
      type: 'point',
      attributes: {
        ball: 'premier' as BallType,
        catchDate: '2023-04-29',
        firstCatch: false,
        game: 'scarlet' as GameType,
        method: 'sandwich' as MethodType,
      },
      relationships: {
        competition: {
          data: {
            id: 'a77269b3-f326-4609-b44b-a804ef7a9f34',
            type: 'competition',
          },
        },
        player: {
          data: {
            id: 'ff4976ce-1519-4ad6-a124-eef49565fc58',
            type: 'player',
          },
        },
        pokemon: {
          data: {
            id: '90a0c67a-ea66-4b05-a603-b1a625e21f64',
            type: 'pokemon',
          },
        },
      },
    },
  },
};
